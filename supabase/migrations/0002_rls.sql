...
-- ============================================================================
-- §5.1 — 헬퍼 함수 (public schema · stable)
-- 기획서 04_PERMISSIONS §5.1 본문은 auth.* 로 정의되어 있으나 Supabase 가
-- auth 스키마 사용자 함수 생성을 차단(SQLSTATE 42501)하므로 public 으로 이동.
-- auth.jwt() · auth.uid() 는 Supabase built-in 이라 호출만 그대로 사용.
-- ============================================================================
...
create or replace function public.technician_id()
returns uuid
language sql stable
as $$
...
language sql stable
as $$
  select public.admin_role() = any(roles);
$$;
...
create policy orders_select_technician on orders for select
  using (
    public.technician_id() is not null
    and assigned_technician_id = public.technician_id()
  );

create policy orders_insert on orders for insert
  with check (public.has_admin_role(array['super_admin','cs_admin','ops_admin']));
...
create policy customers_mutate on customers for all
  using (public.has_admin_role(array['super_admin','cs_admin','ops_admin']))
  with check (public.has_admin_role(array['super_admin','cs_admin','ops_admin']));
...
create policy issues_select_own on issues for select
  using (technician_id = public.technician_id() or public.is_admin());
...
create policy technicians_select_self on technicians for select
  using (id = public.technician_id() or public.is_admin());

create policy technicians_update_self on technicians for update
  using (id = public.technician_id())
  with check (id = public.technician_id());
...
create policy avail_self on technician_availabilities for all
  using (technician_id = public.technician_id())
  with check (technician_id = public.technician_id());
...
create policy avail_read on technician_availabilities for select
  using (public.has_admin_role(array['cs_admin','ops_admin','auditor']));
...
create policy install_update_tech on installations for update
  using (technician_id = public.technician_id() or public.is_admin())
  with check (technician_id = public.technician_id() or public.is_admin());
...
create policy audit_auditor_all on audit_events for select
  using (public.admin_role() in ('auditor','super_admin'));
...
create policy happy_calls_cs_all on happy_calls for all
  using (public.has_admin_role(array['super_admin','cs_admin']))
  with check (public.has_admin_role(array['super_admin','cs_admin']));

create policy happy_calls_admin_read on happy_calls for select
  using (public.has_admin_role(array['dispatch_admin','ops_admin','auditor']));
...
create policy notifications_tech_read on notifications for select
  using (
    ...
    and recipient_id = public.technician_id()
  );
